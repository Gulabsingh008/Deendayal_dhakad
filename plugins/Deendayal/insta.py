...
async def link_handler(Mbot, message):
    ...
    global headers
    try:
        m = await message.reply_sticker("CAACAgUAAxkBAAIDaGdZHL8lZGj9P0L_eFJgqFDk304jAAIgCAACqsfpVVlvjwYdAYWUHgQ CAACAgUAAxkBAAIDbmdZHRXngCghbVkidjGK9lQ5ra74AAJZCQAC02DpVUp7vh9mBK1hHgQ CAACAgUAAxkBAAIDbWdZHRK0AAG1FwaeF8WQ4FVZDKvaAANjBwACV5roVQq3GvTPqAroHgQ")
        ...
        try:
            if "/reel/" in url:
               ...
               try:
                   if ddinsta:
                      dump_file=await message.reply_video(content_value,caption="𝐷𝑜𝑤𝑛𝑙𝑜𝑎𝑑 𝐵𝑦 👉 @AF_Mhakal")
                   else:
                       dump_file=await message.reply_video(content_value, caption="𝐷𝑜𝑤𝑛𝑙𝑜𝑎𝑑 𝐵𝑦 👉 @AF_Mhakal")
               except:
              ...
              #    await message.reply(meta)
                  for i in range(len(meta) - 1):
                     ...
                     try:
                        dump_file=await message.reply_video(com.text,caption="𝐷𝑜𝑤𝑛𝑙𝑜𝑎𝑑 𝐵𝑦 👉 @AF_Mhakal")
                        await com.delete()
                     except:
                         pass 
            elif "stories" in url:
                  ...
                  if meta_tag.ok:
                     ...
                     meta=re.findall(r'href="(https?://[^"]+)"', res['data']) 
                      ...
                      return await message.reply("Oops something went wrong")
                  try:
                     dump_file=await message.reply_video(meta[0], caption="𝐷𝑜𝑤𝑛𝑙𝑜𝑎𝑑 𝐵𝑦 👉 @AF_Mhakal")
                  except:
